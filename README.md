...
## Versão v14 — correção de leitura/RLS do texto oficial

A v14 atua no ponto mais provável da falha de sincronização entre o
administrador e o usuário comum.

### Mudanças no Supabase
Execute `migration_official_text_sync_v14.sql` uma vez.

Ela:
- recria a policy de leitura de `official_page_edits`;
- permite leitura apenas para usuários autenticados aprovados ou admin;
- mantém insert/update/delete somente para admin;
- cria a RPC `get_official_page_edits(module_id)`;
- a RPC usa `SECURITY DEFINER`, mas valida `auth.uid()` e o status `approved`;
- cria trigger para atualizar `updated_at` automaticamente.

### Mudanças no portal
- leitura de textos passa a usar a RPC v14 primeiro;
- se a RPC ainda não existir, há fallback para SELECT direto;
- o botão `🔄 Sincronizar` informa quantos itens oficiais foram lidos;
- publicação do admin é confirmada por releitura independente;
- o texto oficial continua sincronizando automaticamente enquanto o módulo estiver aberto.

## Versão v15 — cursor estável durante edição

Corrige o cursor voltando ao início do texto. A sincronização automática de
4 segundos não reaplica mais o HTML enquanto o administrador está em modo
edição. O rascunho continua sendo salvo, e a sincronização volta ao normal
quando a edição termina. Não é necessário novo SQL; a migration v14 continua
válida.


## Versão v16 — separação dos controles de usuário e administrador

No perfil de usuário comum foram removidos do rodapé do visualizador:
- Modo edição
- Restaurar lâmpadas
- Salvar texto
- Sincronizar
...
Os botões de publicação já eram exclusivos do administrador e continuam assim.

No modo usuário permanecem no rodapé apenas:
- Anterior
- Favoritar
- Marcar como concluído
- Próximo

No perfil de administrador todos os controles anteriores permanecem disponíveis,
sem alteração no fluxo de edição, publicação, sincronização ou lâmpadas.

A sincronização automática de conteúdo oficial continua funcionando para o
usuário comum em segundo plano, mesmo sem o botão manual "Sincronizar".

Não é necessário novo SQL. A migration v14 continua válida.
