...
function captureCurrentChildContent(){
  ...
  if(!itemId){
    const active=doc.querySelector(
      '.lamp.active[data-id],.hotspot.active[data-id],.quick.active[data-id],[data-id].active'
    );
    itemId=active?.dataset?.id || null;
  }
...
function initializeIframeContentBridge(){
  ...
  // Guarda o item já no pointerdown, antes que qualquer lógica da one page
  // possa interromper o clique.
  const rememberFromEvent=(ev)=>{
    const trigger=ev.target.closest?.('[data-id]');
    const id=trigger?.dataset?.id;
    if(id) rememberItem(id);
  };
  doc.addEventListener('pointerdown',rememberFromEvent,true);
  ...
  // Captura qualquer botão equivalente a "Salvar alterações".
  const saveCandidates=[...doc.querySelectorAll('button,input[type="button"],input[type="submit"]')]
    .filter(el=>{
      if(el.id==='saveBtn') return true;
      const txt=(el.textContent||el.value||'').trim().toLowerCase();
      return txt.includes('salvar');
    });

  const saveTextDraft=()=>{
    // Tentativas curtas cobrem páginas que atualizam o DOM no próprio clique.
  ...
  saveCandidates.forEach(saveBtn=>{
    if(saveBtn.dataset.u39TextSaveBound==='1') return;
    saveBtn.dataset.u39TextSaveBound='1';
    saveBtn.addEventListener('click',saveTextDraft,true);
  });

  // Se o usuário edita e sai do campo sem clicar em salvar, mantemos a
  // referência do item para que "Publicar edição" ainda consiga capturá-lo.
  doc.addEventListener('input',ev=>{
    const panel=getChildPanel(doc);
    if(panel?.body && (ev.target===panel.body || panel.body.contains(ev.target))){
...
async function publishCurrentOfficialEdit(){
  ...
  showToast('Texto oficial publicado para todos.');
  updateOfficialEditUI();
}
