...
function getChildPanel(doc){
  if(!doc) return null;
  const body=doc.querySelector('#panelBody,.panelbody,.panel-body');
  if(!body) return null;
...
function getCurrentChildItemId(){
  ...
  const active=doc.querySelector('.quick.active[data-id],.hotspot.active[data-id],button.lamp.active[data-id],.lamp.active[data-id]');
...
async function publishCurrentOfficialEdit(){
  ...
  const moduleId=currentModuleId();
  if(!moduleId || !current){
    showToast('Abra uma informação da one page antes de publicar.','err');
    return;
  }
