...
async function loadOfficialModuleData(moduleId,{applyCurrent=false,silent=false}={}){
  ...
  if(editsRes.error || lampsRes.error){
    ...
    if(/does not exist|schema cache|relation/i.test(message)){
      ...
      if(!silent) showToast('Execute migration_official_edits.sql no Supabase para ativar a publicação oficial.','err');
    }else{
      console.warn('Sincronização de edições oficiais:',editsRes.error,lampsRes.error);
      if(!silent) showToast('Falha ao sincronizar conteúdo oficial.','err');
    }
    updateOfficialEditUI();
  ...
  // Detecta mudança de conteúdo oficial sem depender de cache do navegador.
  ...
  if(applyCurrent && currentModuleId()===moduleId){
    ...
    // Atualiza também o layout oficial das lâmpadas; posição pessoal,
    // quando existente, continua tendo prioridade.
    if(lampChanged && iframeLampEditor?.items?.length){
...
async function forceOfficialSync(showMessage=true){
  ...
  const result=await loadOfficialModuleData(moduleId,{applyCurrent:true,silent:!showMessage});
  if(result?.ok && showMessage){
    showToast(result.changed ? 'Conteúdo oficial atualizado.' : 'Conteúdo oficial já está atualizado.');
  }
  return !!result?.ok;
}
...
function captureCurrentChildContent(){
  ...
  let itemId=getCurrentChildItemId();

  // Fallback: procura a lâmpada ativa diretamente.
  if(!itemId){
    const active=doc.querySelector(
      '[data-u39-item-id].active,.lamp.active[data-id],.hotspot.active[data-id],' +
      '.quick.active[data-id],.quick-card.active[data-id],[data-id].active'
    );
    itemId=active?.dataset?.u39ItemId || active?.dataset?.id || null;
  }
...
function effectiveEditFor(itemId){
  ...
  if(personal && official){
    ...
    // Uma publicação oficial nova não pode ficar escondida por um rascunho
    // antigo existente no celular do usuário.
...
function initializeIframeContentBridge(){
  ...
  const scheduleApply=(itemId,delay=230)=>{
    ...
    rememberItem(itemId);
    setTimeout(async()=>{
      // Busca a publicação mais recente antes de mostrar o texto.
      const moduleId=currentModuleId();
      if(moduleId){
  ...
  // Botões especiais que não usam data-id.
  const summaryBtn=doc.querySelector('#summaryBtn');
  if(summaryBtn){
    summaryBtn.dataset.u39ItemId='summary';
    summaryBtn.addEventListener('pointerdown',()=>rememberItem('summary'),true);
  ...
  if(openFlowBtn){
    ...
    openFlowBtn.addEventListener('pointerdown',()=>rememberItem('flow'),true);
    openFlowBtn.addEventListener('click',()=>scheduleApply('flow'),true);
  }

  // Botão nativo Salvar alterações: funciona em todas as famílias.
  ...
  if(saveBtn && saveBtn.dataset.u39TextSaveBound!=='1'){
    ...
    saveBtn.addEventListener('click',()=>{
      // Captura antes e depois do script nativo.
      saveCurrentTextDraft(false);
      setTimeout(()=>{
        if(!saveCurrentTextDraft(true)){
          showToast('Não foi possível localizar o texto aberto.','err');
        }
      },80);
    },true);
  }

  // Autosave do rascunho durante a digitação.
  doc.addEventListener('input',ev=>{
    const currentStandard=normalizeChildPageToU39Standard(doc);
    const textBody=currentStandard?.body;
    if(!textBody) return;
  ...
  // Garante que a edição central seja aplicada já na carga.
  setChildTextEditMode(lampEditMode);
  updateOfficialEditUI();
}
...
async function publishCurrentOfficialEdit(){
  ...
  // Confirma que o registro realmente pode ser relido do Supabase.
  ...
  if(verifyError || !verify || verify.content_html!==current.html){
    showToast('A gravação não pôde ser confirmada no Supabase.','err');
  ...
  showToast('Texto confirmado no Supabase e publicado para todos.');
  updateOfficialEditUI();
}
...
function setViewerLampEdit(on){
  ...
  if(ed?.doc?.body){
    ...
    // Padrão único: o portal força o texto editável diretamente e,
    // quando existe, sincroniza também o botão nativo.
    setChildTextEditMode(lampEditMode);
  }
  ...
  if(iframeLampEditor){
    showToast(lampEditMode
      ? 'Modo edição ativo: texto e lâmpadas liberados.'
